...
__adddv(Chernarus);
#endif
diag_log format ["Hohei Evolution 2 - Version Settings: %1 #######", e_version];
...
bis_fnc_halo = compile preprocessFileLineNumbers "client\fn_halo.sqf";
#endif

#ifdef __BLUE__
"respawn_west" setMarkerPos [(getMarkerPos "base_spawn_1" select 0),(getMarkerPos "base_spawn_1" select 1)];
#else
"respawn_east" setMarkerPos [(getMarkerPos "base_spawn_1" select 0),(getMarkerPos "base_spawn_1" select 1)];
#endif
...
switch (E_diffparam) do {
	case 1 : {// Easy
		EVO_Infantry = [2,10];		// [Actual number of !groups!, Base number of !men!]
...
//define vars--------------------------------------------------------------------
EVO_rank1 = 10;							//points still needed for the rank
EVO_rank2 = 20;
EVO_rank3 = 40;
EVO_rank4 = 80;
EVO_rank5 = 125;
...
EVO_AHQ_respawn_delay = 240;			//time in seconds
EVO_MHQ_respawn_delay = 240;			//time in seconds
EVO_jetspawn_delay = 300;			//time in seconds
EVO_attackhspawn_delay = 300;			//time in seconds

EVO_town_free_score = 10;				//score for every player still in town in moment of clearing
EVO_radio_score = 15;					//score for destroying the radio tower
EVO_radio_score_near = 5;				//score ffor be present inner a cirlce of 100metres by destroying radio tower
EVO_officer_score = 4;					//scorepoints, one time for capture and again for transport to prison
EVO_soldier_score = 2;					//scorepoints for capture an unamred soldier

EVO_ai_skill = [0.3,0.2];				//first number: minimum ai skill, second number random additional
EVO_max_alive_infs = 4;					//max numbers from alive enemys to call a maintarget "clear"
...
EVO_all_possible_towns = [
	...
	[[6016.37,5690.92,0],"Anar",300], // 10
	[[5304.67,4742.74,0],"Kakaru",300], // 11
	[[3655.71,4316.29,0],"Sakhe",300], // 12
	[[1466.8,3594.07,0],"Shukurkalay",300], // 13
	[[546.094,2811.05,0],"Chaman",300], // 14
	[[8894.68,5272.33,0],"Timurkalay",300], // 15
	[[4438.04,686.898,0],"Chak Chak",300], // 16
	[[6041.86,1148.34,0],"Huzrutimam",300], // 17
	[[8485.58,2452.54,0],"Loy Manara",300], // 18
	[[10142.7,2336.75,0],"Chardarakht",300], // 19
	[[2003.28,352.347,0],"Landay",300], // 20
	[[1987.14,7657.36,0],"Mulladost",300], // 21
	[[11528.4,8351.98,0],"Ravanay",300], // 22
	[[1507.13,5701.05,0],"Khushab",300] // 23
];
#else
EVO_all_possible_towns = [	
...
switch (true) do {
	...
	case (__A2only) : {
		EVO_pammo = ["30Rnd_556x45_Stanag","30Rnd_556x45_Stanag","30Rnd_556x45_Stanag","30Rnd_556x45_Stanag","30Rnd_556x45_Stanag","Laserbatteries","15Rnd_9x19_M9","15Rnd_9x19_M9","15Rnd_9x19_M9","15Rnd_9x19_M9"];
		EVO_pweapons = ["M4A1_Aim","M9","Laserdesignator","NVGoggles"];
	};
	...
	case (__COver and __Chernarus and !__ACEver) : {
		EVO_pammo = ["30Rnd_556x45_Stanag","30Rnd_556x45_Stanag","30Rnd_556x45_Stanag","30Rnd_556x45_Stanag","30Rnd_556x45_Stanag","17Rnd_9x19_glock17","17Rnd_9x19_glock17","17Rnd_9x19_glock17","17Rnd_9x19_glock17"];
		EVO_pweapons = ["M4A1_Aim","glock17_EP1","Binocular_Vector","NVGoggles"];
	};
...
switch (true) do {
	case (__OAonly) : {
		...
		EVO_pweapons = ["AK_107_PSO","Makarov","Binocular","NVGoggles"];
	};
	case (!__ACEver and (__A2only or __COver)) : {
		EVO_pammo = ["30Rnd_545x39_AK","30Rnd_545x39_AK","30Rnd_545x39_AK","30Rnd_545x39_AK","30Rnd_545x39_AK","pipebomb","Laserbatteries","8Rnd_9x18_Makarov","8Rnd_9x18_Makarov","8Rnd_9x18_Makarov","8Rnd_9x18_Makarov"];
		EVO_pweapons = ["AK_107_PSO","Makarov","Laserdesignator","NVGoggles"];
	};
	case (__ACEver and __Takistan) : {
...
#ifdef __ACE__
	EVO_pitems = ["ItemCompass","ItemGPS","ACE_Map","ACE_Map_Tools","ItemRadio","ItemWatch","ACE_Earplugs"];
#else
	EVO_pitems = ["ItemCompass","ItemGPS","ItemMap","ItemRadio","ItemWatch"];
#endif
...
#else
EVO_rank1_vecs = ["Mi17_CDF","HMMWV_Armored","HMMWV_M2","HMMWV_TOW","HMMWV_MK19","Mi17_Civilian","LAV25","AAV"];
Evo_Rank2_vecs = ["HMMWV_Avenger","MH60S","M1A1","T72_CDF","ZSU_CDF"];
EVO_rank3_vecs = ["MLRS","M1A2_TUSK_MG","UH1Y","MV22"];
EVO_rank4_vecs = ["M119","AH64D","AH1Z","AH64D_Sidewinders","C130J"];
EVO_rank5_vecs = ["A10","Su25_CDF"];
EVO_rank6_vecs = ["AV8B","AV8B2","F35B","F-35B"];
#endif
...
if __isBlue then {
		...
		EVO_e_wheeled = ["UAZ_AGS30_RU","GRAD_RU","GAZ_Vodnik_HMG","GAZ_Vodnik","BTR90"];
		EVO_e_tracked = ["T90","T72_RU","BMP3"];
		EVO_e_aa = ["2S6M_Tunguska"];
		EVO_e_jets = ["Su34","Su39"];
		EVO_e_attackhelis = ["Mi17_rockets_RU","Mi24_V","Mi24_P","Ka52"];
		EVO_e_transporthelis = ["Mi17_medevac_RU"];
		...
		EVO_f_tracked = ["MLRS","M1A2_TUSK_MG","M1A1","AAV"];
		...
		EVO_e_tracked = ["MLRS","M1A2_TUSK_MG","M1A1","AAV"];
		...
		EVO_f_wheeled = ["UAZ_AGS30_RU","GRAD_RU","GAZ_Vodnik_HMG","GAZ_Vodnik","BTR90"];
		EVO_f_tracked = ["T90","T72_RU","BMP3"];
		EVO_f_aa = ["2S6M_Tunguska"];
		EVO_f_jets = ["Su34","Su39"];
		EVO_f_attackhelis = ["Mi17_rockets_RU","Mi24_V","Mi24_P","Ka52"];
		EVO_f_transporthelis = ["Mi17_medevac_RU"];
...
"BAF_FV510_D","BAF_ATV_D","BAF_ATV_W","M113Ambul_UN_EP1"]; //include all ace-types, BAF, PMC, A2 and OA
#else
d_helilift1_types = ["T72_TK_EP1","T55_TK_EP1","T34_TK_EP1","BMP2_HQ_TK_EP1","BMP2_TK_EP1","M113_TK_EP1","BMP2_Ambul_INS",
...
init_lift_choppers = {
	...
	} forEach [[ch1,301],[ch2,302],[ch3,303],[ch4,304],[ch5,305],[ch6,306]];
};
// editor varname, unique number, true = respawn only when the chopper is completely destroyed, false = respawn after some time when no crew is in or chopper is destroyed
if (isServer) then {
	[[ch1,301,false,120],[ch2,302,false,120],[ch3,303,false,120],[ch4,304,false,120],[ch5,305,false,120],[ch6,306,false,120]] execVM "server\x_helirespawn2.sqf";
};
