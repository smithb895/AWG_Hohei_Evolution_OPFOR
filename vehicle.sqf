...
_unitinit = if (count _this > 6) then {'this addeventhandler ["fired",{  if (vehicle (_this select 0) in list base ) then {deleteVehicle (_this select 6)}; }];  '} else {'this addeventhandler ["fired",{  if (vehicle (_this select 0) in list base ) then {deleteVehicle (_this select 6)}; }];  '};_haveinit = if (count _this > 6) then {true} else {true};
...
{	
	...
	{	
		...
		_unit setVehicleInit format ["%1;", 'this addeventhandler ["fired",{  if (vehicle (_this select 0) in list base ) then {deleteVehicle (_this select 6)}; }];'];
		processInitCommands;
		_dead = false;
	};
};